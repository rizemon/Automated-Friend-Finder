...
def age_not_in_range(naMe):
    ...
    for name in profiles.keys():
        ...
        if profiles[name]['gender'] != profiles[naMe]['gender']:
            ...
            for Age_Range in (
            range((profiles[naMe]['acceptable_age_range']['start']), (profiles[naMe]['acceptable_age_range']['end']))):
    ...
    for name in profiles.keys():
        ...
        if profiles[name]['gender'] != profiles[inputName]['gender']:
            ...
            if name != inputName:
                ...
                # Compare everyone's age with the person's acceptable age range
                for acc_Age_Range in (range((profiles[inputName]['acceptable_age_range']['start']),
                                            (profiles[inputName]['acceptable_age_range']['end']))):
...
def bothLikesDislikesCompatibility(inputedPersonName):
    ...
    bothLikesDislikes = dict(
        dislikesToLikes.items() + likesToDislikes.items() + [(k, dislikesToLikes[k] + likesToDislikes[k]) for k in
                                                             set(likesToDislikes) & set(dislikesToLikes)])
...
def overallCompatibility(name):
    ...
    Temp1 = dict((countryCompatibility(name)).items() + (ageCompatibility(name)).items() + [
        (k, (countryCompatibility(name))[k] + (ageCompatibility(name))[k]) for k in
        set(ageCompatibility(name)) & set(countryCompatibility(name))])
    Temp2 = dict(
        Temp1.items() + (likesCompatibility(name)).items() + [(k, (Temp1)[k] + (likesCompatibility(name))[k]) for k in
                                                              set(likesCompatibility(name)) & set(Temp1)])
    Temp3 = dict(
        Temp2.items() + (dislikesCompatibility(name)).items() + [(k, (Temp2)[k] + (dislikesCompatibility(name))[k]) for
                                                                 k in set(dislikesCompatibility(name)) & set(Temp2)])
    overallCompatibility = dict(Temp3.items() + (bothLikesDislikesCompatibility(name)).items() + [
        (k, (Temp3)[k] + (bothLikesDislikesCompatibility(name))[k]) for k in
        set(bothLikesDislikesCompatibility(name)) & set(Temp3)])
